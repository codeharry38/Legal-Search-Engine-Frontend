<template>
    <v-container fluid>
        <v-row>
            <v-col lg=6 sm=6 md=6>
                <v-chip label color="blue-grey darken-3" dark>
                    <v-icon left small>mdi-account-multiple</v-icon> Parties
                </v-chip>
            </v-col>
        ...
        <v-dialog v-model="editBox" scrollable persistent max-width="500px" transition="dialog-transition">
            <party :edit="true" :id="selectedId"></party>
        </v-dialog>
...
export default {
    components:{party,delConfirm,pagination},
    mounted(){
        this.fetchrecords();
    },
    data(){
        return{
            // Dlete Url Is here
            delUrl:SERVER_API+'/party/delete',
            fab:false,
            header:[
    ...
    methods:{
        async fetchrecords(){
            try{
                this.isLoading =  true;                
                const response = await this.$http.get(SERVER_API+'/party/records?page='+this.currentPage);
