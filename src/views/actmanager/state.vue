<template>
    <v-container fluid>
        <v-row>
            <v-col lg=6 sm=6 md=6>
                <v-chip label color="blue-grey darken-3" dark>
                    <v-icon left small>mdi-routes</v-icon> Act's State
                </v-chip>
            </v-col>
        ...
        <v-dialog v-model="editBox" scrollable persistent max-width="500px" transition="dialog-transition">
            <state :edit="true" :id="selectedId"></state>
        </v-dialog>
...
import state from '../../components/act/state.vue'
import Config from '../../config'
export default {
    components:{state,delConfirm},
    mounted(){
        this.fetchrecords();
    },
    data(){
        return{
            // Dlete Url Is here
            delUrl:SERVER_API+'/act/state/delete',
            fab:false,
            header:[
