        ...
        <v-row>
            <v-col lg=6 sm=6 md=6>
                <v-chip label color="blue-grey darken-3" dark>
                    <v-icon left small>mdi-chart-pie</v-icon> Act's Section
                </v-chip>
            </v-col>
                ...
                <v-card>
                    ...
                    item-key="_id" loading="true" :search="search">
                        <template v-slot:[`item.action`]="{ item }">
                            <v-btn color="blue-grey darken-3" fab small @click="selectedId=item._id; editBox=true;" dark text><v-icon>mdi-pencil</v-icon></v-btn>
                        </template>
                    </v-data-table>
                </v-card>
            </v-col>
...
import Actsection from '../../components/act/section.vue'
import Config from '../../config'
export default {
    components:{Actsection, delConfirm},
    mounted(){
        this.fetchrecords();
    },
    data(){
        return{
            // Dlete Url Is here
            delUrl:SERVER_API+'/act/section/delete',
            fab:false,
            header:[
                {text:'Section',value:'name'},
                {text:'Short',value:'sName'},
