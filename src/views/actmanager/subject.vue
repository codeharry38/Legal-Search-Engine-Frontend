<template>
    <v-container fluid>
        <v-row>
            <v-col lg=6 sm=6 md=6>
                <v-chip label color="blue-grey darken-3" dark>
                    <v-icon left small>mdi-view-headline</v-icon> Act's Subject
                </v-chip>
            </v-col>
        ...
        <v-dialog v-model="editBox" scrollable persistent max-width="500px" transition="dialog-transition">
            <subject :edit="true" :id="selectedId"></subject>
        </v-dialog>
...
import subject from '../../components/act/subject.vue'
import Config from '../../config'
export default {
    components:{subject,delConfirm},
    mounted(){
        this.fetchrecords();
    },
    data(){
        return{
            // Dlete Url Is here
            delUrl:SERVER_API+'/act/subject/delete',
            fab:false,
            header:[
