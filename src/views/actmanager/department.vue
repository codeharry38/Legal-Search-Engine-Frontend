<template>
    <v-container fluid>
        <v-row>
            <v-col lg=6 sm=6 md=6>
                <v-chip label color="blue-grey darken-3" dark>
                    <v-icon left small>mdi-home-modern</v-icon> Act's Department
                </v-chip>
            </v-col>
        ...
        <v-dialog v-model="editBox" scrollable persistent max-width="500px" transition="dialog-transition">
            <department :edit="true" :id="selectedId"></department>
        </v-dialog>
...
import department from '../../components/act/department.vue'
import Config from '../../config'
export default {
    components:{department,delConfirm},
    mounted(){
        this.fetchrecords();
    },
    data(){
        return{
            // Dlete Url Is here
            delUrl:SERVER_API+'/act/department/delete',
            fab:false,
            header:[
