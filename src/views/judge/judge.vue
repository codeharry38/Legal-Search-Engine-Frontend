<template>
    <v-container fluid>
        <v-row>
            <v-col lg=6 sm=6 md=6>
                <v-chip label color="blue-grey darken-3" dark>
                    <v-icon left small>mdi-gavel</v-icon> Judges
                </v-chip>
            </v-col>
        ...
        <v-row>
            <v-col cols=12>
                <v-card>
                    ...
                    <v-data-table v-model="selection" :headers="header" :items="records" show-select dense class="tableTitleCustom"
                    item-key="_id" :loading="isLoading" :search="search" hide-default-footer disable-pagination>
        ...
        <v-dialog v-model="editBox" scrollable persistent max-width="500px" transition="dialog-transition">
            <judge :edit="true" :id="selectedId"></judge>
        </v-dialog>
...
export default {
    components:{judge,delConfirm,pagination},
    mounted(){
        this.fetchrecords();
    },
    data(){
        return{
            // Dlete Url Is here
            delUrl:SERVER_API+'/judge/delete',
            fab:false,
            header:[
                {text:'Title', value:'prefix'},
                {text:'Judge Name',value:'name'},
    ...
    methods:{
        async fetchrecords(){
            try{
                this.isLoading =  true;                
                const response = await this.$http.get(SERVER_API+'/judge/records?page='+this.currentPage);
