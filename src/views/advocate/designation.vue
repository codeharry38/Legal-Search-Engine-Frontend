<template>
    <v-container fluid>
        <v-row>
            <v-col lg=6 sm=6 md=6>
                <v-chip label color="blue-grey darken-3" dark>
                    <v-icon left small>mdi-account-star </v-icon> Advocate's Designations
                </v-chip>
            </v-col>
        ...
        <v-row>
            <v-col cols=12>
                <v-card>
                    ...
                    item-key="_id" loading="true" :search="search" dense class="tableTitleCustom">
        ...
        <v-dialog v-model="editBox" scrollable persistent max-width="500px" transition="dialog-transition">
            <desgn :edit="true" :id="selectedId"></desgn>
        </v-dialog>
...
import desgn from '../../components/advocate/designation.vue'
import Config from '../../config'
export default {
    components:{desgn,delConfirm},
    mounted(){
        this.fetchrecords();
    },
    data(){
        return{
            // Dlete Url Is here
            delUrl:SERVER_API+'/advocate/desgn/delete',
            fab:false,
            header:[
