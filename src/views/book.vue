        ...
        <v-row>
            <v-col lg=6 sm=6 md=6>
                <v-chip label color="blue-grey darken-3" dark>
                    <v-icon left small>mdi-book-open-variant</v-icon> Books
                </v-chip>
            </v-col>
...
export default {
    components:{book,delConfirm,pagination},
    mounted(){
        this.fetchrecords();
    },
    data(){
    ...
    methods:{
        async fetchrecords(){
            try{
                this.isLoading =  true;                
                const response = await this.$http.get(SERVER_API+'/book/records?page='+this.currentPage);
