<template>
    <v-container fluid>
        <v-row>
            <v-col lg=6 sm=6 md=6>
                <v-chip label color="blue-grey darken-3" dark>
                    <v-icon left small>mdi-bank</v-icon> Courts
                </v-chip>
            </v-col>
        ...
        <v-row>
            <v-col cols=12>
                <v-card>
                    ...
                    item-key="_id" :loading="isLoading" :search="search" hide-default-footer disable-pagination dense class="tableTitleCustom">
        ...
        <v-dialog v-model="editBox" scrollable persistent max-width="500px" transition="dialog-transition">
            <Court :edit="true" :id="selectedId"></Court>
        </v-dialog>
...
import Court from '../components/court/index.vue'
import Config from '../config'
import pagination from '../components/pagination/index.vue'
export default {
    components:{Court,delConfirm,pagination},
    mounted(){
        this.fetchrecords();
    },
    data(){
        return{
            // Dlete Url Is here
            delUrl:SERVER_API+'/court/delete',
            fab:false,
            header:[
                {text:'Court Name',value:'name'},
                {text:'Short Name',value:'sName'},
    ...
    methods:{
        async fetchrecords(){
            try{
                this.isLoading =  true;                
                const response = await this.$http.get(SERVER_API+'/court/records?page='+this.currentPage);
