<template>
    <v-container fluid>
        <v-row>
            <v-col lg=6 sm=6 md=6>
                <v-chip label color="blue-grey darken-3" dark>
                    <v-icon left small>mdi-format-list-bulleted-type</v-icon> Case Types
                </v-chip>
            </v-col>
        ...
        <v-dialog v-model="editBox" scrollable persistent max-width="500px" transition="dialog-transition">
            <type :edit="true" :id="selectedId"></type>
        </v-dialog>
...
import type from '../../components/case/type.vue'
import Config from '../../config'
export default {
    components:{type,delConfirm},
    mounted(){
        this.fetchrecords();
    },
    data(){
        return{
            // Dlete Url Is here
            delUrl:SERVER_API+'/case/type/delete',
            fab:false,
            header:[
                {text:'Case Type',value:'name'},
