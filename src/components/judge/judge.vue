    ...
    <v-card>
        <v-app-bar  dark color="blue-grey darken-3" dense flat>
            <v-toolbar-title class="dialogTitle">
                {{edit == true ? `Edit Judge` : `New Judge`}}
            </v-toolbar-title>
            <v-spacer></v-spacer>
        ...
        <v-form @keyup.enter.native="Save()">
            <v-card-text>
                <v-autocomplete dense v-model="prefix" label="Judge Title" :items="titles" item-text="prefix" item-value="_id" 
                outlined color="blue-grey darken-3"
                ></v-autocomplete>
                <v-text-field dense v-model="name" label="Judge Name" outlined color="blue-grey darken-3"></v-text-field>
            </v-card-text>
    ...
    export default {
        ...
        methods:{
            ...
            async fetchbyId(){
                if(this.edit == true){
                    ...
                    try{
                        ...
                        const Response = await this.$http.get(SERVER_API+'/judge/findbyid/'+this.id);
            ...
            async create(){
                try{
                    this.isLoading = true;
                    const Response = await this.$http.post(SERVER_API+'/judge/create',{
                        prefix:this.prefix,
                        name:this.name,
                        status:this.status
                    });
                    ...
                    this.reset();
                    bus.$emit('response',{res: Response.data, type:'create',model:'judge'});
                    this.close();
                }catch(error){
                    this.isLoading = false;
                    bus.$emit('error',error);
            ...
            async update(){
                try{
                    this.isLoading = true;
                    const Response = await this.$http.put(SERVER_API+'/judge/update',{
                        id:this.id,
                        prefix:this.prefix,
                        name:this.name,
                        status:this.status
                    ...
                    this.isLoading = false;
                    bus.$emit('response',{res: Response.data, type:'update',model:'judge'});
                    this.close();
                }catch(error){
                    this.isLoading = false;
                    bus.$emit('error',error);
