    ...
    <v-card>
        <v-app-bar  dark color="blue-grey darken-3" dense flat>
            <v-toolbar-title class="dialogTitle">
                {{edit == true ? `Edit Judge's Designation` : `New Judge's Designation`}}
            </v-toolbar-title>
            <v-spacer></v-spacer>
        ...
        <v-form @keyup.enter.native="Save()">
            <v-card-text>
                <v-text-field dense v-model="name" label="Judge's Designation" outlined color="blue-grey darken-3"></v-text-field>
    ...
    export default {
        ...
        methods:{
            ...
            async fetchbyId(){
                if(this.edit == true){
                    ...
                    try{
                        ...
                        const Response = await this.$http.get(SERVER_API+'/judge/desgn/findbyid/'+this.id);
            ...
            async create(){
                try{
                    this.isLoading = true;
                    const Response = await this.$http.post(SERVER_API+'/judge/desgn/create',{
                        sName: this.sName,
                        name:this.name,
                        status:this.status
                    });
            ...
            async update(){
                try{
                    this.isLoading = true;
                    const Response = await this.$http.put(SERVER_API+'/judge/desgn/update',{
                        id:this.id,
                        sName:this.sName,
                        name:this.name,
                        status:this.status
