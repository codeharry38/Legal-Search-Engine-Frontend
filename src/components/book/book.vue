<template>
    <v-card>
        ...
        <v-form @keyup.enter.native="Save()">
            <v-card-text>
                <v-text-field v-model="name" label="Book Name" outlined color="blue-grey darken-3" dense></v-text-field>
            </v-card-text>
    ...
    export default {
        ...
        methods:{
            ...
            async fetchbyId(){
                if(this.edit == true){
                    ...
                    try{
                        ...
                        const Response = await this.$http.get(SERVER_API+'/book/findbyid/'+this.id);
            ...
            async create(){
                try{
                    this.isLoading = true;
                    const Response = await this.$http.post(SERVER_API+'/book/create',{
                        name:this.name,
                        status:this.status
                    });
                    this.isLoading = false;
                    this.reset();
                    bus.$emit('response',{res: Response.data, type:'create',model:'book'});
                    this.close();
                }catch(error){
                    this.isLoading = false;
                    bus.$emit('error',error);
            ...
            async update(){
                try{
                    this.isLoading = true;
                    const Response = await this.$http.put(SERVER_API+'/book/update',{
                        id:this.id,
                        name:this.name,
                        status:this.status
                    });
                    this.isLoading = false;
                    bus.$emit('response',{res: Response.data, type:'update',model:'book'});
                    this.close();
                }catch(error){
                    this.isLoading = false;
                    bus.$emit('error',error);
