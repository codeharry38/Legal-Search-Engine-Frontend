<template>
    <v-card>
        <v-app-bar  dark color="blue-grey darken-3" dense flat>
            <v-toolbar-title>
                {{edit == true ? `Edit Act's Department` : `New Act's Department`}}
            </v-toolbar-title>
            <v-spacer></v-spacer>
        ...
        <v-form @keyup.enter.native="Save()">
            <v-card-text>
                <v-text-field v-model="name" label="Act's Department" outlined color="blue-grey darken-3"></v-text-field>
            </v-card-text>
    ...
    export default {
        ...
        methods:{
            ...
            async fetchbyId(){
                if(this.edit == true){
                    ...
                    try{
                        ...
                        const Response = await this.$http.get(SERVER_API+'/act/department/findbyid/'+this.id);
            ...
            async create(){
                try{
                    this.isLoading = true;
                    const Response = await this.$http.post(SERVER_API+'/act/department/create',{
                        name:this.name,
                        status:this.status
                    });
                    this.isLoading = false;
                    this.reset();
                    bus.$emit('response',{res: Response.data, type:'create',model:'department'});
                    this.close();
                }catch(error){
                    this.isLoading = false;
                    bus.$emit('error',error);
            ...
            async update(){
                try{
                    this.isLoading = true;
                    const Response = await this.$http.put(SERVER_API+'/act/department/update',{
                        id:this.id,
                        name:this.name,
                        status:this.status
                    });
                    this.isLoading = false;
                    bus.$emit('response',{res: Response.data, type:'update',model:'department'});
                    this.close();
                }catch(error){
                    this.isLoading = false;
                    bus.$emit('error',error);
