<template>
    <v-card>
        <v-app-bar  dark color="blue-grey darken-3" dense flat>
            <v-toolbar-title>
                {{edit == true ? `Edit Act's State` : `New Act's State`}}
            </v-toolbar-title>
            <v-spacer></v-spacer>
        ...
        <v-form @keyup.enter.native="Save()">
            <v-card-text>
                <v-text-field v-model="name" label="Act's State" outlined color="blue-grey darken-3"></v-text-field>
            </v-card-text>
    ...
    export default {
        ...
        methods:{
            ...
            async fetchbyId(){
                if(this.edit == true){
                    ...
                    try{
                        ...
                        const Response = await this.$http.get(SERVER_API+'/act/state/findbyid/'+this.id);
            ...
            async create(){
                try{
                    this.isLoading = true;
                    const Response = await this.$http.post(SERVER_API+'/act/state/create',{
                        name:this.name,
                        status:this.status
                    });
                    this.isLoading = false;
                    this.reset();
                    bus.$emit('response',{res: Response.data, type:'create', model:'state'});
                    this.close();
                }catch(error){
                    this.isLoading = false;
                    bus.$emit('error',error);
            ...
            async update(){
                try{
                    this.isLoading = true;
                    const Response = await this.$http.put(SERVER_API+'/act/state/update',{
                        id:this.id,
                        name:this.name,
                        status:this.status
                    });
                    this.isLoading = false;
                    bus.$emit('response',{res: Response.data, type:'update', model:'state'});
                    this.close();
                }catch(error){
                    this.isLoading = false;
                    bus.$emit('error',error);
